...
export function DroppingJulyBanner() {
  ...
  async function handleNotify(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/waitlist", {
        ...
        body: JSON.stringify({ email, source: "dropping-july" }),
      })
              ...
              {!submitted ? (
                <>
                  <div className="text-center mb-5">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">Enter your email to be the first to know when the new collection drops.</p>
                  </div>
                  ...
                  <p className="text-[10px] text-muted-foreground text-center mt-3">No spam. Only new drop alerts.</p>
                </>
              ) : (
