...
interface Props {
  products: any[]
  autoScroll?: boolean
  /** "grid" renders a static responsive grid instead of a horizontal scroll carousel. */
  layout?: "carousel" | "grid"
}

export function ProductCarousel({ products, autoScroll = false, layout = "carousel" }: Props) {
  ...
  const cards = products.map((product) => {
    ...
    return (
      <div key={product._id} className={layout === "grid" ? "" : "flex-shrink-0 w-[44vw] md:w-[22vw] lg:w-[18vw]"}>
  ...
  if (layout === "grid") {
    return (
      <div className="container">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">{cards}</div>
      </div>
    )
  }
